...
contract CreatorHubTest is Test {
    ...
    function testCreatePaidContent() public {
        vm.prank(creator);
        uint256 contentId =
            hub.createContent(CreatorHub.ContentType.ARTICLE, "ipfs://paid", false, 10 ether, 2 ether, paymentToken);

        (,,, bool isFree, uint256 fullPrice, uint256 rentedPrice, address token,) = hub.contents(contentId);
    ...
    function testRevertIfCreatorNotRegisteredOrInactive() public {
        ...
        hub.createContent(CreatorHub.ContentType.VIDEO, "ipfs://fail", true, 0, 0, address(0));
    }

    function testRevertIfEmptyMetadata() public {
        vm.prank(creator);
        vm.expectRevert("Invalid metadataURI");
        hub.createContent(CreatorHub.ContentType.VIDEO, "", true, 0, 0, address(0));
    }

    function testRevertInvalidFreePricing() public {
        vm.prank(creator);
        vm.expectRevert("Free content fullPrice must be 0");
        hub.createContent(CreatorHub.ContentType.VIDEO, "ipfs://bad", true, 1 ether, 0, address(0));
    }

    function testRevertInvalidPaidPricing() public {
        vm.prank(creator);
        vm.expectRevert("Payment token required");
        hub.createContent(CreatorHub.ContentType.VIDEO, "ipfs://bad", false, 5 ether, 1 ether, address(0));
    }
    ...
    function testUpdateContent() public {
        vm.prank(creator);
        uint256 contentId = hub.createContent(CreatorHub.ContentType.VIDEO, "ipfs://old", true, 0, 0, address(0));

        vm.prank(creator);
        hub.updateContent(
            contentId, CreatorHub.ContentType.PODCAST, "ipfs://new", 10 ether, 2 ether, paymentToken, false
        );
    ...
    function testUpdateContentRevertsIfNotCreator() public {
        vm.prank(creator);
        uint256 contentId = hub.createContent(CreatorHub.ContentType.VIDEO, "ipfs://x", true, 0, 0, address(0));

        vm.prank(stranger);
        vm.expectRevert("Only the content creator can update");
        hub.updateContent(contentId, CreatorHub.ContentType.ARTICLE, "ipfs://hack", 0, 0, address(0), true);
    }

    function testUpdateNonExistentContentReverts() public {
        vm.prank(creator);
        vm.expectRevert("Content does not exist");
        hub.updateContent(999, CreatorHub.ContentType.VIDEO, "ipfs://nope", 0, 0, address(0), true);
    }
    ...
    function testSetContentActive() public {
        vm.prank(creator);
        uint256 contentId = hub.createContent(CreatorHub.ContentType.VIDEO, "ipfs://content", true, 0, 0, address(0));
